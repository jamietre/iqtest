...
u.promise.resolve();}}function m(){var u=this;i.call(this,this,"setup");this.reset();
w.each(u.tests,function(A,B){B.reset();});this.doWriterEvent("groupStart",w.filterProps(u,"name,desc"));
w.each(u.tests,function(C,E){var A=w.extend({},E.assert,{test:E}),D=w.extend({},E.refute,{test:E});
...
t(u,E);});});return this;}function j(A,u){var B=this;w.each(this.group.writers,function(D,C){var E=C[A];
...
};this.clear();};v.prototype={constructor:v,add:k,run:m,then:function(){return this.promise.then.apply(this,w.toArray(arguments));
...
u.clear();this.doWriterEvent=function(){j.apply(this,w.toArray(arguments));};};r.prototype={constructor:r,impl:{},reset:function(){w.extend(this,{promise:y.defer(),results:[],count:0,countPassed:0,countFailed:0,nextThen:[],cbPromise:null,resolver:null,stopped:false,passed:null});
this.promise.resolve();},clear:function(){this.setDebug(false);},setDebug:function(u,A){this.debug=w.isBool(u)?u:true;
...
},E=C.promise,D=y.defer();C.promise=D;E.then(function(G){try{if(C.nextIsProblemAssertion()){;
}u(G);}catch(F){C.testerror("An error occurred during a 'then' clause of an assertion: "+String(F),true);
}},A);y.chain(E,D);return C;},chain:function(u,A){var B=y.defer(),C=this.promise;
this.promise=B.promise;C.then(u,A||this.testerror);y.chain(C,B);return this;},afterNext:function(u,A){this.nextThen.push({callback:u,errback:A});
...
}else{I.reject("The test failed");}},function L(N){if(F.itemRunning){F.endTest({passed:false,err:String(N)});
...
u.count=this.count;u.fulltext=w.format("Test #{0} {1} {2} {3}{4}",this.count,this.assertionInfo.assertion,A,u.passed?"":": "+B.err,w.format(' in test "{0}"',this.assertionInfo.desc));
...
u.reject(F);return;}}}u.resolve(F);};},when:function(u,D){var C=D||this.timeout,B=y.defer(),A=this.promise;
this.promise=C?z(B,C*1000):B;A.then(function(){u.call(this,B.resolve);});return this;
},backpromise:function(D,u,G){var B=y.defer(),E=this,F=G||E.timeout,A=function(){var I;
if(u){if(E.debug){I=u.apply(this,w.toArray(arguments));}else{try{I=u.apply(this,w.toArray(arguments));

...
*/
(function(b){var d=b.impl.utility,c={group:'<pre><h1>Starting test group "<iq-name></iq-name>": <iq-groupresult></iq-groupresult></h1><div><div></div></div></pre>',testStart:'<h2>Starting test "<iq-name></iq-name>": <iq-testresult></iq-testresult></h2><div></div>',testEnd:"<h3><iq-count></iq-count> assertions passed.</h3>",itemStart:'<span style="color: blue;font-style: italic;">#<iq-count></iq-count>: <iq-assertion></iq-assertion> "<iq-desc></iq-desc>"....</span><br/>',itemEnd:"<span><iq-fulltext></iq-fulltext></span><br/>",resultSuccess:'<span style="color:green"><iq-resulttext></iq-resulttext></span>',resultFail:'<span style="color:red"><iq-resulttext></iq-resulttext></span>',log:'<span style="color:purple"><iq-message></iq-message></span><br/>'};
...
this.outputTarget=f;return this;},groupStart:function(e){var f=d.tmpReplace($(c.group).clone(),e);
...
},itemEnd:function(e){if(e.passed){if(!this.showPassed){this.itemTarget.remove();
}}else{this.itemTarget.replaceWith(d.tmpReplace($(c.itemEnd).clone(),e));}this.itemTarget=null;
e.written=true;},log:function(e){this.target.append(d.tmpReplace($(c.log).clone(),{message:e}));

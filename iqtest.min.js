...
j.event(p.testStart,p,r);r.func.call(r,r);o=r.promise;l=function(){if(r.promise!==o){o=r.promise;
...
}return l;}},then:function(){return this.promise.then.apply(this.promise,j.toArray(arguments));
...
o=function(t){return p.runTest.call(p,"fail",{desc:m.desc,err:t.toString()});};r=function(t){m.actual=t;
};s=function(){return p.runTest.call(p,n,m);};k=when.isPromise(m.actual);if(p.cbPromise){if(k){return o("The actual value passed is a promise, but a callback has also been initiated");
}p.cbPromise.then(r,o);q.push(p.cbPromise);p.cbPromise=null;}p.promise.then(function(){p.startTest({assertArgs:m,assertFuncName:n});
});if(k){m.actual.then(r,o);q.push(m.actual);}if(q.length){q.push(p.promise);p.promise=when.all(q);
}p.promise.then(s,o);return p;},callback:function(m,n){var l=this,k=when.defer();
l.cbPromise=when_timeout(k,n?n*1000:10000);return function(){var o=!m?true:m.apply(l,j.toArray(arguments));
k.resolve(o);};},backpromise:function(m,k,r){var n=when.defer(),o=this,q=when_timeout(n,r?r*1000:10000),l=function(){n.resolve(k.apply(this,j.toArray(arguments)));

...
(function(a){a(["./iqtest"],function(w,y,z,p,e,x){var q,r,v,b,l={name:"test-group",desc:"Unnamed Test Group",debug:false,showPassed:false,setup:null,teardown:null},s={name:"test",desc:"Unnamed Test",func:null,showPassed:false,timeoutSeconds:10,debug:false},f="same,equals,typeOf,defined,isNull,match,isObject,isFunction,exception,tagName,className,isTrue,isFalse",o=false,c={};
...
},resolves:function(C,B){return{passed:typeof C!=="undefined",err:w.formatAssert(B,"The object {0} did {not}resolve",String(C))};
...
B=arguments[2];C=!!B;F={name:E,desc:C?A:"",func:C?B:A,debug:D.debug,showPassed:D.showPassed};
...
},timeout:function(B){var u=this,A=u.timeoutSeconds;u.then(function(){u.timeoutSeconds=B;
});u.afterNext(function(){u.timeoutSeconds=A;});},configure:function(A){if(typeof A==="string"){A={name:A};
}var u=w.extend({},s);w.extend(u,this,A,true);w.extend(this,u);},then:function(u,B){var C=this,A=B||function(F){C.testerror(F,false);
},E=C.promise,D=y.defer();D.then(function(){try{if(C.nextIsProblemAssertion()){;}u();
}catch(F){C.testerror("An error occurred during a 'then' clause of an assertion: "+String(F),true);
}},A);C.promise=D;y.chain(E,D);return C;},afterNext:function(u,A){this.nextThen.push({callback:u,errback:A});
},startTest:function(u){this.count++;this.doWriterEvent("itemStart",w.extend({},u,{count:this.count}));
this.itemRunning=true;},endTest:function(A){var u=A;if(!this.itemRunning){this.testerror("Error: test was not running when endTest called: "+A.desc);
...
}N.then(function(P){u[O]=P;});J.push(N);}});F.then(function(){F.startTest({desc:d(A,u),assertion:A});
});if(J.length){J.push(F.promise);F.promise=F.timeoutSeconds?z(D.promise,F.timeoutSeconds*1000):D.promise;
...
D.passed=false;}this.endTest(D);return D.passed;},addResult:function(B){var A=w.extend({},B),u=A.passed?"passed":"failed";
A.count=this.count;A.fulltext=w.format("Test #{0} {1}{2}{3}",this.count,u,A.passed?"":": "+B.err,B.desc?w.format(' in test "{0}"',B.desc):"");
this.results.push(A);return A;},testerror:function(A,u){var B=this;if(B.stopped){return;
}B.stopped=true;B.passed=false;this.doWriterEvent("testLog",w.format("{0}. {1}",String(A),u?"Debugging is enabled if you start again.":""));
if(u){B.setDebug(true,B.count);}},callback:function(C,D){var A=this,B=D||A.timeoutSeconds,u=y.defer();
...
u.reject(F);return;}}}u.resolve(F);};},backpromise:function(D,u,G){var B=y.defer(),E=this,F=G||E.timeoutSeconds,A=function(){var I;

...
return{passed:!!u,err:o.formatAssert("The object {0} is {not}truthy",u,t)};}});s=l.prototype;
r=buster.assertions;o.each(["assert","refute"],function(u,v){var t={};o.each(f.split(","),function(w,x){g(x,r[v][x]);
t[x]=function(){var y=((this.test&&this.test instanceof l)?this.test:this);return y.queueTest(r[v][x],v+"."+x,o.toArray(arguments));
};});o.each(k,function(w,x){o.each(x,function(z,y){if(!t[z]){g(z,y);t[z]=function(){var A=((this.test&&this.test instanceof l)?this.test:this);
return A.queueBooleanTest(y,v+"."+z,o.toArray(arguments),v==="refute");};}});});if(v==="assert"){o.extend(s,t);
}o.each(["backpromise","callback","then"],function(w,x){t[x]=function(){return this.test[x].apply(this.testObject,o.toArray(arguments));
...
},trim:function(r){return r.replace(/^\s\s*/,"").replace(/\s\s*$/,"");},format:function(s){var r=(arguments.length===2&&o.isArray(arguments[1]))?arguments[1]:this.toArray(arguments,1);
...
}},formatAssert:function(t,s,r){return(r?r+": ":"")+o.format(t,String(s)).replace("{not}","{0}");
...
if(v.passed===t){throw ({name:"AssertionError",type:"iq",message:o.format(v.err,t?"not":"")});
...
s.cbPromise=u?q(r,u*1000):r.promise;return function(){var x;if(!v){x=true;}else{if(s.debug){x=v.apply(this,o.toArray(arguments));

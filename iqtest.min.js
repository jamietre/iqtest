...
}},formatAssert:function(s,u,t){return !u?"":(s?s+": ":"")+t?o.format(u,o.isArray(t)?t:o.toArray(arguments,2)):"";
}};n=function(s){i();this.tests=[];o.extend(this,o.extend(null,h,s,true));this.promise=q.defer();
this.passed=null;};n.prototype={constructor:n,test:function(w,s,t){var v=this,u=!!t,y={name:w,desc:u?s:"",func:u?t:s,debug:v.debug,showPassed:v.showPassed},x=new l(y);
v.tests.push(x);x.group=v;x.id=v.tests.length;return v;},run:function(w,s,u){var t,v=this;
o.each(v.tests,function(x,y){y.reset();});o.event(v.groupStart,v,o.filter(v,"name,desc"));
o.each(v.tests,function(z,B){var x=o.extend({},B.assert,{test:B}),A=o.extend({},B.assert,{test:B});
...
}}B._lastPromise=B.promise;B._allPass=true;t=function(){if(B.promise!==B._lastPromise){B._lastPromise=B.promise;
B._lastPromise.then(t,t);return;}if(!o.isBool(B.passed)){B.passed=(B.count===B.countPassed);
}B._allPass&=B.passed;o.event(B.testEnd,B,o.filter(B,"count,passed"));if(B===v.tests[v.tests.length-1]){if(o.isBool(B.passed)){v.passed=B._allPass;
}o.event(v.groupEnd,v,o.filter(B,"passed"));}};B.promise.then(t,t);});},then:function(u,s){if(o.isFunction(u)){this.promise=this.promise.then.apply(this.promise,o.toArray(arguments));
return this;}else{var t=new n(this);this.promise=this.promise.then(function(){t.test(u,s);
return t;},function(){alert("debug: failback called on a group");});return t;}},configure:function(t){var s=o.extend({},h);
o.extend(s,t,true);o.extend(this,s);return this;},groupStart:o.donothing,groupEnd:o.donothing};
l=function(t){var s=this;o.extend(s,m);o.extend(s,t,true);s.id=null;s.group=null;
s.reset();};l.prototype={constructor:l,impl:{},reset:function(){o.extend(this,{promise:q.defer(),results:[],count:0,countPassed:0,countFailed:0,nextThen:[],cbPromise:null,resolver:null,stopped:false,passed:null});
this.promise.resolve();},timeout:function(u){var s=this,t=s.timeoutSeconds;s.then(function(){s.timeoutSeconds=u;
});s.afterNext(function(){s.timeoutSeconds=t;});},configure:function(t){var s=o.extend({},m);
o.extend(s,this,t,true);o.extend(this,s);},then:function(s,t){var v=this,u=t||function(x){v.testerror(x,false);
},w=v.promise;v.promise=q.defer(s,u);q.chain(w,v.promise);return v;},afterNext:function(s,t){this.nextThen.push({callback:s,errback:t});
},startTest:function(s){this.count++;o.event(this.itemStart,this,o.extend({},s,{count:this.count}));
this.itemRunning=true;},endTest:function(t){var s=t;if(!t.passed){s=this.addResult(t);
this.countFailed++;}else{this.countPassed++;}this.itemRunning=false;o.event(this.itemEnd,this,s);
},queueBooleanTest:function(v,t,s,u){return this.queueTest(function(){var w=v.apply(null,o.toArray(arguments));
if(w.passed===u){throw ({name:"AssertionError",type:"iq",message:o.format(w.err.replace("{not}","{0}"),u?"not ":"")});
}},t,s);},queueTest:function(z,t,s){var x=this,u,y=c[t.split(".")[1]].args,w,v,A,B=[],C;
if(x.cbPromise){w=true;v=v||q.defer();if(y===1){u=0;}else{if(s[0]&&!s[1]){u=1;}else{if(s[1]&&!s[0]){u=0;
}else{x.testerror(o.format("I couldn't figure out what to do with your magic callback. For this test you may need to define it explicitly.[{0}] {1}",t,d(s)));
return;}}}x.cbPromise.then(function(D){s[u]=D;},function(D){v.reject("The callback failed. "+(D?o.format("Reason: {0}",String(D)):""));
});x.cbPromise=null;}o.each(s,function(E,D){if(q.isPromise(D)){v=v||q.defer();if(E===0&&w){v.reject("You're using magic callback but you've also defined a promise as the first argument of your assert.");
}D.then(function(F){s[E]=F;});B.push(D);}});x.then(function(){x.startTest({desc:d(t,s),assertion:t});
});if(B.length){B.push(x.promise);x.promise=v.promise;q.chain(q.all(B),v);}A=q.defer();
C=x.promise;x.promise=A.promise;C.then(function(){if(x.runTest.call(x,z,t,s)){A.resolve();
}else{A.reject("The test failed.");}},function(){alert("error");});return x;},runTest:function(v,t,s){var w={assertion:t,err:"",passed:true};
try{v.apply(null,s);}catch(u){if(u.name!=="AssertionError"){if(this.debug){;v.apply(null,s);
}else{this.debug=true;u.message=(u.message||u.type)+". Debugging has been enabled.";
}}if(u.type==="iq"){u.message=o.format("[{0}] {1}",t,u.message);}w.err=u.message;
w.passed=false;}this.endTest(w);return w.passed;},addResult:function(u){var t=o.extend({},u),s=t.passed?"passed":"failed";
t.count=this.count;t.fulltext=o.format("Test #{0} {1}{2}{3}",this.count,s,t.passed?"":": "+u.err,u.desc?o.format(' in test "{0}"',u.desc):"");
this.results.push(t);return t;},testerror:function(t,s){var v=this;if(v.resolver){try{v.resolver.reject(t);
}catch(u){}v.resolver=null;}if(v.stopped){return;}v.stopped=true;v.passed=false;o.event(v.log,v.group,o.format("{0}. {1}",String(t),s?"Debugging is enabled if you start again.":""));
if(s){v.debug=true;}},callback:function(w,x){var u=this,v=x||u.timeoutSeconds,s=q.defer();
u.cbPromise=v?r(s,v*1000):s;return function(){var y;if(!w){y=true;}else{if(u.debug){y=w.apply(this,o.toArray(arguments));
}else{try{y=w.apply(this,o.toArray(arguments));}catch(t){u.testerror("An error occurred in your callback(): "+t,true);
...
return{test:function(){var s=new n(this);return s.test.apply(s,o.toArray(arguments));
},extend:function(s){k.push(s);},impl:{TestGroup:n,Test:l,Assert:b,utility:o}};});

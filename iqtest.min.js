...
o.extend(r,this,s,true);o.extend(this,r);},then:function(r,t){var v=this,u=t||function(x){v.testerror(x,false);
},s=p.defer(),w=v.promise;v.promise=s.promise;w.then(function(){r();s.resolve();},function(){s.reject();
u();});return v;},afterNext:function(r,s){this.nextThen.push({callback:r,errback:s});
...
}},s,r);},queueTest:function(y,s,r){var w=this,t,x=c[s.split(".")[1]].args,v,u,z,A=[],B;
if(w.cbPromise){v=true;u=u||p.defer();if(x===1){t=0;}else{if(r[0]&&!r[1]){t=1;}else{if(r[1]&&!r[0]){t=0;
...
return;}}}w.cbPromise.then(function(C){r[t]=C;},function(C){u.reject("The callback failed. "+(C?o.format("Reason: {0}",String(C)):""));
});A.push(w.cbPromise);w.cbPromise=null;}o.each(r,function(D,C){if(p.isPromise(C)){u=u||p.defer();
if(D===0&&v){u.reject("You're using magic callback but you've also defined a promise as the first argument of your assert.");
}C.then(function(E){r[D]=E;});A.push(C);}});w.then(function(){w.startTest({desc:d(s,r),assertion:s});
});if(A.length){A.push(w.promise);w.promise=u.promise;p.chain(p.all(A),u);}z=p.defer();
B=w.promise;w.promise=z.promise;B.then(function(){if(w.runTest.call(w,y,s,r)){z.resolve();
}else{z.reject("The test failed.");}},function(){alert("error");});return w;},runTest:function(u,s,r){var v={assertion:s,err:"",passed:true};
try{u.apply(null,r);}catch(t){if(t.name!=="AssertionError"){if(this.debug){;u.apply(null,r);
}else{this.debug=true;t.message=(t.message||t.type)+". Debugging has been enabled.";
}}if(t.type==="iq"){t.message=o.format("[{0}] {1}",s,t.message);}v.err=t.message;
v.passed=false;}this.endTest(v);return v.passed;},addResult:function(t){var s=o.extend({},t),r=s.passed?"passed":"failed";

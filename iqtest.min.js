...
v.tests.push(x);x.group=v;x.id=v.tests.length;return v;},add:function(t){var s=this;
if(t.constructor===n){o.each(t.tests,function(v,u){s.tests.push(u);});}else{if(t.constructor===l){s.tests.push(t);
}}return s;},run:function(w,s,u){var t,v=this;o.each(v.tests,function(x,y){y.reset();
});o.event(v.groupStart,v,o.filter(v,"name,desc"));o.each(v.tests,function(z,B){var x=o.extend({},B.assert,{test:B}),A=o.extend({},B.assert,{test:B});
...
C._lastPromise.then(function(){t(C);},function(){t(C);});return;}if(!o.isBool(C.passed)){C.passed=(C.count===C.countPassed);
}C._allPass&=C.passed;o.event(C.testEnd,C,o.filter(C,"count,passed"));if(C===v.tests[v.tests.length-1]){if(o.isBool(C.passed)){v.passed=C._allPass;
}o.event(v.groupEnd,v,o.filter(C,"passed"));}};B.promise.then(function(){t(B);},function(){t(B);
});});},then:function(u,s){if(o.isFunction(u)){this.promise=this.promise.then.apply(this.promise,o.toArray(arguments));
...
o.extend(s,this,t,true);o.extend(this,s);},then:function(s,u){var v=this,t=u||function(y){v.testerror(y,false);
},x=v.promise,w=q.defer();w.then(s,t);v.promise=w;q.chain(x,w);return v;},afterNext:function(s,t){this.nextThen.push({callback:s,errback:t});
...
if(x.stopped){return x;}if(x.cbPromise){w=true;v=v||q.defer();if(y===1){u=0;}else{if(s[0]&&!s[1]){u=1;
}else{if(s[1]&&!s[0]){u=0;}else{x.testerror(o.format("I couldn't figure out what to do with your magic callback. For this test you may need to define it explicitly.[{0}] {1}",t,d(s)));
return;}}}x.cbPromise.then(function(F){s[u]=F;},function(F){v.reject("The callback failed. "+(F?o.format("Reason: {0}",String(F)):""));
});B.push(x.cbPromise);x.cbPromise=null;}o.each(s,function(G,F){if(q.isPromise(F)){v=v||q.defer();
if(G===0&&w){v.reject("You're using magic callback but you've also defined a promise as the first argument of your assert.");
}F.then(function(H){s[G]=H;});B.push(F);}});x.then(function(){x.startTest({desc:d(t,s),assertion:t});
});if(B.length){B.push(x.promise);x.promise=v.promise;q.chain(q.all(B),v);}A=q.defer();
C=x.promise;x.promise=A.promise;C.then(function E(F){if(x.runTest.call(x,z,t,s)){A.resolve();
}else{A.reject("The test failed");}},function D(F){A.reject(F);});return x;},runTest:function(v,t,s){var w={assertion:t,err:"",passed:true};
...
}u.stopped=true;u.passed=false;o.event(u.log,u.group,o.format("{0}. {1}",String(t),s?"Debugging is enabled if you start again.":""));
